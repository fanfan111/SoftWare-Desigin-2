...
NTSTATUS AllocateVMXRegion()
{
	...
	//IO_BITMAP 8kb
	pIOBitmap = ExAllocatePoolWithTag(NonPagedPool,8192,'test');
	if (!pIOBitmap)
	{
		Log("ERROR:申请IOBitmap内存区域失败!",0);
		ExFreePoolWithTag(pIOBitmap,8192);
		return STATUS_MEMORY_NOT_ALLOCATED;
	}
	//全置zero
	RtlZeroMemory(pIOBitmap,8192);
	//0x60H置1
	((unsigned char*)pIOBitmap)[12] = 0x01;
	...
	g_VMXCPU[uCPUID].pIOBitmap = pIOBitmap;
	g_VMXCPU[uCPUID].pIOBitmap_PA = MmGetPhysicalAddress(pIOBitmap);
...
extern "C" void SetupVMCS()
{
	...
	*((PULONG64)&uEflags) = Asm_GetRflags();
	...
	//拦截IO
	uCPUBase |= CPU_BASED_USE_IO_BITMAPS;
	...
	//IO位图
	Vmx_VmWrite(IO_BITMAP_A, g_VMXCPU[uCPUID].pIOBitmap_PA.QuadPart);
	Vmx_VmWrite(IO_BITMAP_B, g_VMXCPU[uCPUID].pIOBitmap_PA.QuadPart + 4096);
...
NTSTATUS StartVirtualTechnology()
{
	...
	for (int i = 0;i<KeNumberProcessors;i++)
	{
		...
		Log("正在这个处理器上开启VT",KeGetCurrentProcessorNumber());
...
NTSTATUS StopVirtualTechnology()
{
	...
	{
		KeSetSystemAffinityThread((KAFFINITY)(1 << i));
		Log("正在这个处理器上关闭VT",KeGetCurrentProcessorNumber());
